...
To avoid building the kernel each time, I build each kernel once in CI, and let every box download the result. That's what [nix-pvm](https://github.com/fcjr/nix-pvm) is. It defines both kernels in [Nix](https://nixos.org/); CI rebuilds them every week and pushes them to a cache ([Cachix](https://www.cachix.org/)). Each box pulls the prebuilt kernel.
...
The PVM host kernel is minimal and doesn't ship the default initrd modules (`nvme`, `pata_marvell`, and so on), so the stock module closure fails to build. On Hetzner Cloud you only need virtio to reach the disk, and the `qemu-guest` profile handles that, so in `configuration.nix`:
...
The initramfs has to be gzip. The guest kernel currently only enables `CONFIG_RD_GZIP`, so anything else won't unpack:
...
And bam, the host boots on the PVM kernel, `/dev/kvm` is live, and the supervisor boots a single-use microVM per job.

## What I'm Using It For

The whole detour was for a programming contest I'm running this summer with some friends: [Nolen Royalty](https://eieio.games) (our fearless leader), plus [Eliot Hedeman](https://github.com/eliothedeman) and [Henry Nelson](https://github.com/hcnelson99). More on that when we launch.

A contest means running code from strangers to score it. We want a clean, disposable VM around every submission, and we want a lot of them to be cheap. nix-pvm is how we get it.
...
Or wire it into your flake with the steps above. The cache contians both prebuilt kernels, so neither ever compiles on your box.

Source is on [GitHub](https://github.com/fcjr/nix-pvm). It's early and I've only tested it on one environment (Linux 6.7.12 on a Hetzner `cx23`), but it should run anywhere and give you a real VM, on a cloud box that normally can't run one.
